...
public class RTree<T> {
	...
	private void visualize(Node n, java.io.PrintWriter pw, int x0, int y0, int w, int h) {
		...
		pw.println("<span style=\"word-wrap:break-word;\">");
		pw.println("Node: " + n.toString() + " (root==" + (n == root) + ")<br>");
		pw.println("Coords: " + Arrays.toString(n.coords) + "<br>");
		pw.println("Dimensions: " + Arrays.toString(n.dimensions) + "<br>");
		pw.println("# Children: " + ((n.children == null) ? 0 : n.children.size()) + "<br>");
		pw.println("isLeaf: " + n.leaf + "<br>");
		pw.println("</span>");
		int numChildren = (n.children == null) ? 0 : n.children.size();
		for (int i = 0; i < numChildren; i++) {
			visualize(n.children.get(i), pw, (int) (x0 + (i * w / (float) numChildren)), y0 + elemHeight, (int) (w / (float) numChildren), h - elemHeight);
		}
		pw.println("</div>");
	}
}
